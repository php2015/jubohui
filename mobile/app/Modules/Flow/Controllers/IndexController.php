...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	{
		// print_r(__PUBLIC__.'/img/jz.jpg');exit;
		if(!empty($_GET['is_real'])){
			$users = dao('users')->where('user_name = '.$_GET['username'])->find();
			// print_r($_GET['total_stage']);exit;
			// $_SESSION['total_stage'] = $_GET['total_stage'];
			if(!empty($_GET['Jsdate'])){
				...
				if(!empty($Jsdate)){
					...
					$Cart->is_real = 1;
					if($Jsdate['cart_img']){
						$Cart->cart_img = $Jsdate['cart_img'];
					}else{
						$Cart->cart_img = __PUBLIC__.'/img/jz.jpg';
					}
					
					$Cart->capital_id = $Jsdate['id'];
					$Cart->total_stage = $_GET['total_stage'];
					$Cart->goods_number = 1;
					$Cart->add_time = time();
					$result = $Cart->add();
					if(!$result){
			...
			$this->assign('total_stage', $_GET['total_stage']);
		}
		...
		// print_r($cart_goods_list_new);exit;
...
', '', $GLOBALS['_CFG']['invoice_content']));
	...
	{
		// print_r($_POST['total_stage']);exit;
		$order_time = gmtime();
		...
		$total_stage = !empty($_POST['total_stage']) ?  $_POST['total_stage'] : 0 ;
