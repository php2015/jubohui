...
class LoginController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	// 注册接口
	public function actionRegisterInter()
	{	
		// print_r($_GET);exit;
		if (IS_GET) {
			...
			if (empty($username)) {
				exit(json_encode(array('status' => 'n', 'info' => '用户名不能为空')));
			}

			if (empty($password)) {
				exit(json_encode(array('status' => 'n', 'info' => '密码不能为空')));
			}
			...
			if (!empty($users)) {
				exit(json_encode(array('status' => 'n', 'info' => '已存在该账号')));
			}
