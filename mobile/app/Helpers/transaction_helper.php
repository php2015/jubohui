...
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0)
{
	...
	foreach ($res as $key => $row) {
		...
		// 添加状态
		$row['state_flow'] = stateFlow($row['state']);
		// print_r($row['state_flow']);exit;
		$br = '';
		$order_over = 0;
		if ($row['user_order'] == OS_SPLITED && $row['user_shipping'] == SS_RECEIVED && $row['user_pay'] == PS_PAYED) {
			$order_over = 1;
...
function stateFlow($state){
	...
	if(!empty($state)){
		if($state == 1){
			$stageLast['sf'] = '项目经理已入住';
		}
		if($state == 2){
			$stageLast['sf'] = '项目经理申报已验收';
		}
		if($state == 3){
			$stageLast['sf'] = '质检已确认';
		}
		...
		$stageLast['sf'] = '项目经理未入住';
	}
	return $stageLast;
}

// 备用
function stateFlow_($state){
	$stageLast = array();
	if(!empty($state)){
		if($state == 1){
