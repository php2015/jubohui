...
$url_ = 'http://47.111.64.44:8080/BaiJiaJia/addModel.do?method=addPinPai_sc';
/**
 * curl请求
...
if ($_REQUEST['act'] == 'list') {
	...
	if ($brand_id = $db->insert_id()) {
		// 传值到APP
		$data = array(
			'pinpaiName' => $_POST['brand_name'],
			'scId' => $brand_id,
			'stype' => 2
		);
		// 提交
		$ret = doCurl($url_, $data);
		// 轉數組
		$rdata = json_decode($ret, true);
		// print_r($rdata);exit;
		if($rdata['code'] == '1' && !empty($rdata)){
			$sql = 'UPDATE' . $ecs->table('brand') . (' SET bpp_id=' . $rdata['id'] . ' WHERE brand_id = '.$brand_id);
			$res_ = $db->query($sql);
			if(!$res_){
				$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('brand') . (' WHERE brand_id = \'' . $brand_id . '\'');
				$GLOBALS['db']->query($sql);
				$link[] = array('href' => 'brand.php?act=list');
				sys_msg('保存bpp_id失败',1,$link);
			}
		}else{
			$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('brand') . (' WHERE brand_id = \'' . $brand_id . '\'');
			$GLOBALS['db']->query($sql);
			$link[] = array('href' => 'brand.php?act=list');
			sys_msg('获取bpp_id失败',1,$link);
		}
