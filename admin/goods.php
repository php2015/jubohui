...
// 新增和修改商品链接
$url_1 = 'http://47.111.64.44:8080/BaiJiaJia/addModel.do?method=addModel_sc';

// 商品上下架链接
$url_2 = 'http://47.111.64.44:8080/BaiJiaJia/addModel.do?method=editModelState_sc';
			...
			if ($_REQUEST['act'] == 'insert_link_desc' || $_REQUEST['act'] == 'update_link_desc') {
				...
				if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
					...
					if($goods_id){						
						...
						// print_r($data);exit;
						// 提交
						$ret = doCurl($url_1, $data);
						
						// 轉數組
						$rdata = json_decode($ret, true);
						// print_r($rdata);exit;
						if($rdata['code'] == '1' && !empty($rdata)){
							$sql = 'UPDATE' . $ecs->table('goods') . (' SET ppid=' . $rdata['id'] . ' WHERE goods_id = '.$goods_id);
							$res_ = $db->query($sql);
							if(!$res_){
								$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('goods') . (' WHERE goods_id = \'' . $goods_id . '\'');
								$GLOBALS['db']->query($sql);
								$link[] = array('href' => 'goods.php?act=add');
								sys_msg('保存ppid失败',1,$link);
							}
						}else{
							$sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('goods') . (' WHERE goods_id = \'' . $goods_id . '\'');
							$GLOBALS['db']->query($sql);
							$link[] = array('href' => 'goods.php?act=add');
							sys_msg('获取ppid失败',1,$link);
						}
					}
                ...
                WHERE g.goods_id = \'' . $goods_id . '\'');
					...
					if ($exc->edit('is_on_sale = \'' . $on_sale . '\', last_update=' . gmtime(), $goods_id)) {
						...
						if($rdata['code'] == '1' && !empty($rdata)){
							...
							clear_cache_files();
							make_json_result($on_sale,0,$rdata);
						}else{
							make_json_result($on_sale,0,$rdata);
						}
